...
if [[ -f "./${DOTDIR}/setup_${platform}.sh" ]]; then
  if [[ -f "./${DOTDIR}/setup_${platform}.installed" ]]; then
    echo "System setup already adjusted, delete ./${DOTDIR}/setup_${platform}.installed to run setup again."
  else
...
if [[ -f "./${DOTDIR}/system_${platform}.sh" ]]; then
  ...
  echo "System settings already adjusted, delete ./${DOTDIR}/system_${platform}.installed to run system script again."
  else
